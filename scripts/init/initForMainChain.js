...
const main = async () => {
  ...
  */
  ...
  //   NFTCollection
  // */
  const RandNFTCollection = await ethers.getContractFactory("MainNFTCollection");
  const rdNFTCollection = await RandNFTCollection.attach(process.env.MAIN_NFT_COLLECTION);
  ...
  */
  const setApprovalForAll = await rdOwnerNFTCollection.setApprovalForAll(process.env.MAIN_BRIDGE, true, {gasLimit: BigInt(1e7)});
  await setApprovalForAll.wait();
  console.log("1: ", setApprovalForAll);
  console.log("2: ", await rdOwnerNFTCollection.isApprovedForAll(adminKey.publicKey, process.env.MAIN_BRIDGE));
};
